$(function () {
    ...
    _ul.on('click', 'li', function () {
        ...
        var opend = _ul.find('[data-status=1]').length;
        var tmp;
        if(ds == 1){
            tmp = arr.splice(start,1)[0];
            tmp.dataStatus = 0;
            ...
            arr.splice(opend,0,tmp);
        }

        moveLis(arr);
        ds ?
            (ds = 0, $(this).find('.slide').removeClass('on'))
            :
            (ds = 1, $(this).find('.slide').addClass('on'));
